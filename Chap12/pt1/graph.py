...
class LinkedDirectedGraph(AbstractCollection):
    ...
    def addVertex(self, label):
        ...
        if self.containsVertex(label):
            raise AttributeError("Label is already in the graph")
    ...
    def getVertex(self, label):
        if not (self.containsVertex(label)):
            raise AttributeError("Label is not already in the graph.")
    ...
    def addEdge(self, fromLabel, toLabel, weight):
        ...
        if not ((self.containsVertex(fromLabel)) and (self.containsVertex(toLabel))):
            raise AttributeError("Vertices are not already in the graph.")

        if self.containsEdge(fromLabel, toLabel):
            raise AttributeError("Vertices are already connected")
