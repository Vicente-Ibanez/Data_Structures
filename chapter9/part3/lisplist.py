...
def removeAll(lyst1, item):
    if first(lyst1) == item:
        return removeAll(rest(lyst1), item)
    else: 
        return cons(first, removeAll(rest(lyst1), item))

def lispMap(func, lyst):
    if isEmpty(lyst):
        return THE_EMPTY_LIST
    return cons(func(first(lyst)), lispMap(func, rest(lyst)))

def lispFilter(func, lyst):
    if isEmpty(lyst):
        return THE_EMPTY_LIST
    if func(first(lyst)):
        return cons(first(lyst), lispFilter(func, rest(lyst)))
    return lispFilter(func, rest(lyst))
...
def main():
    ...
    print(lispMap(lambda x: x ** 2, lyst))
    print(lispFilter(lambda x: x % 2 == 0, lyst))
    print(lispFilter(lambda x: x % 2 == 0,  lispMap(lambda x: x ** 2, lyst)))
