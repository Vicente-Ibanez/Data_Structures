...
class ArraySortedListIterator(object):
    ...
    def next(self):
        ...
        if self.modCount != self.backingStore.getModCount():
            raise AttributeError("Cant modify list and iterate")
    ...
    def remove(self):                
        ...
        if self.lastItemPos == -1:
            raise AttributeError("Position is not defined.")
        
        if self.modCount != self.backingStore.getModCount():
            raise AttributeError("Cant modify list and iterate")
        
        pop = self.backingStore.pop(self.lastItemPos)

        if self.lastItemPos < self.cursor : #item was obtained through next
            self.cursor -= 1
        self.modCount += 1
        self.lastItemPos = -1
